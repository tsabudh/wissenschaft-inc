...
import dataPipelinePageData from "@/data/page-data-pipeline.json";

export const metadata: Metadata = metadataMap["/services/data-pipeline"];

export default function DataPipelinePage() {
  const { hero, whoWeBuildFor, coreServices, process, technologyStack, whyChooseUs, cta } =
    dataPipelinePageData.page ? dataPipelinePageData : dataPipelinePageData;
    ...
    <main className="text-text-primary mx-auto flex min-h-screen max-w-7xl flex-col gap-12 px-6 py-16 lg:px-20 dark:text-gray-300">
      ...
      <section className="mb-20 text-center">
        <h1 className="mb-4 text-4xl leading-tight font-extrabold md:text-5xl lg:text-6xl">
          {dataPipelinePageData.hero.heading}
        </h1>
        <p className="text-text-tertiary mx-auto max-w-3xl text-lg md:text-xl">
          {dataPipelinePageData.hero.subheading}
        </p>
        <div className="mt-8 flex items-center justify-center gap-4">
          <Link
            href={dataPipelinePageData.hero.cta.primary.href}
          ...
          >
            {dataPipelinePageData.hero.cta.primary.text}
          </Link>
          <Link
            href={dataPipelinePageData.hero.cta.secondary.href}
          ...
          >
            {dataPipelinePageData.hero.cta.secondary.text}
          </Link>
        </div>
      </section>
        ...
        <ol className="grid grid-cols-1 gap-6 md:grid-cols-3 xl:grid-cols-5">
          {process.steps.map((step) => (
            <li key={step.number} className="bg-surface-secondary rounded-lg p-6 shadow-sm">
              <div className="mb-2 inline-flex h-10 w-10 items-center justify-center rounded-full bg-blue-50 font-semibold text-blue-700">
                {step.number}
              </div>
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
          ...
          <div className="bg-surface-secondary rounded-xl p-8 shadow-sm">
            ...
            <p className="text-md text-text-tertiary mb-4">
              From healthcare data integration to financial transaction processing and IoT event
              streaming across multiple sectors.
            </p>
      ...
      {/* CONTACT / CTA */}
      <section id="contact" className="rounded-3xl bg-blue-700 px-2 py-16 text-center shadow-md">
        <h3 className="mb-4 text-3xl font-bold text-white md:text-4xl dark:text-gray-200">
          {cta.heading}
        </h3>
        <p className="mx-auto mb-6 max-w-2xl text-lg text-blue-100 md:text-xl">{cta.description}</p>
        <div className="flex items-center justify-center gap-3">
          <Link
            href={cta.button.href}
            className="bg-background inline-block rounded-lg px-8 py-3 font-semibold text-blue-600 shadow-lg transition hover:scale-105 hover:bg-gray-300 hover:text-blue-700"
          >
            {cta.button.text}
          </Link>
        </div>
